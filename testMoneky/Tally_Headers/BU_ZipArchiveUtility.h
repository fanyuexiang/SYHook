//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSString;

@interface BU_ZipArchiveUtility : NSObject
{
    NSString *_path;
    void *_zip;
}

+ (id)_dateWithMSDOSFormat:(unsigned int)arg1;
+ (id)_gregorian;
+ (void)zipInfo:(CDStruct_962f7fa6 *)arg1 setDate:(id)arg2;
+ (void)zipInfo:(CDStruct_962f7fa6 *)arg1 setAttributesOfItemAtPath:(id)arg2;
+ (id)_filenameStringWithCString:(const char *)arg1 version_made_by:(unsigned short)arg2 general_purpose_flag:(unsigned short)arg3 size:(unsigned short)arg4;
+ (_Bool)renameFileName:(id)arg1 toNewName:(id)arg2;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2 preserveAttributes:(_Bool)arg3 overwrite:(_Bool)arg4 needRename:(_Bool)arg5 nestedZipLevel:(long long)arg6 password:(id)arg7 error:(id *)arg8 delegate:(id)arg9 progressHandler:(CDUnknownBlockType)arg10 completionHandler:(CDUnknownBlockType)arg11;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2 preserveAttributes:(_Bool)arg3 overwrite:(_Bool)arg4 needRename:(_Bool)arg5 password:(id)arg6 error:(id *)arg7 delegate:(id)arg8 progressHandler:(CDUnknownBlockType)arg9 completionHandler:(CDUnknownBlockType)arg10;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2 overwrite:(_Bool)arg3 needRename:(_Bool)arg4 password:(id)arg5 error:(id *)arg6 delegate:(id)arg7;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2 delegate:(id)arg3;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2 error:(id *)arg3;
+ (_Bool)unzipFileAtPath:(id)arg1 toDestination:(id)arg2;
- (void).cxx_destruct;
- (_Bool)close;
- (_Bool)writeData:(id)arg1 filename:(id)arg2 compressionLevel:(int)arg3 password:(id)arg4 AES:(_Bool)arg5;
- (_Bool)writeFileAtPath:(id)arg1 withFileName:(id)arg2 compressionLevel:(int)arg3 password:(id)arg4 AES:(_Bool)arg5;
- (_Bool)writeFileAtPath:(id)arg1 withFileName:(id)arg2 withPassword:(id)arg3;
- (_Bool)writeFile:(id)arg1 withPassword:(id)arg2;
- (_Bool)writeFolderAtPath:(id)arg1 withFolderName:(id)arg2 withPassword:(id)arg3;
- (_Bool)open;
- (id)initWithPath:(id)arg1;
- (id)init;

@end


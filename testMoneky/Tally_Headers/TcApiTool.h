//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface TcApiTool : NSObject
{
}

+ (id)getCachedOpenID;
+ (id)_findTopViewControllerFromRootVC:(id)arg1;
+ (id)currentViewController;
+ (void)showAlertTitle:(id)arg1 message:(id)arg2 delegate:(id)arg3 cancelBtnTitle:(id)arg4;
+ (id)doDictionaryByParseURL:(id)arg1;
+ (int)doCheckOpenApiId:(id)arg1;
+ (_Bool)isOpenApiSSoLogin:(id)arg1;
+ (_Bool)openUniversallinkIfNeed:(id)arg1;
+ (id)keyForIfNeedUllinkInKeyChain;
+ (id)keyForAppSignTokenInKeyChain;
+ (void)deleteKeychainValueForKey:(id)arg1;
+ (id)encodeUrlscheme:(id)arg1;
+ (id)encodeUllinkFromUrlscheme:(id)arg1;
+ (id)_decodeUrlSchemeFromUllinkV2:(id)arg1;
+ (id)decodeUrlSchemeFromUllink:(id)arg1;
+ (id)keyForLoginDataInKeyChain;
+ (id)webViewResourcePathOf:(id)arg1;
+ (id)resourceBundlePath;
+ (_Bool)notKeyboardOrAlertWindow:(id)arg1;
+ (id)md5:(id)arg1;
+ (id)getAppDisplayName;
+ (id)systemVersionString;
+ (double)systemVersion;
+ (id)random16Str;
+ (int)NSString16toNSNumber:(id)arg1;
+ (id)decodeAccessToken:(id)arg1 key:(id)arg2;
+ (id)TCOSDKdeviceMachine:(id)arg1;
+ (id)deviceMachineString;
+ (id)SDKInfo;
+ (_Bool)isRegistSchemeInfoPlistFile:(id)arg1;
+ (id)getAppDefinePath;
+ (void)MoveWindowAnimation:(id)arg1 animationId:(int)arg2 rect:(struct CGRect)arg3 timeInterVal:(double)arg4 name:(id)arg5 context:(void *)arg6;
+ (id)GetURLWithParam:(id)arg1 withParam:(id)arg2;
+ (void)MoveAllWindowWithAnimationActionType:(int)arg1 windowArray:(id)arg2 animation:(_Bool)arg3;
+ (void)MoveAllWindowIntoScreen:(id)arg1 animation:(_Bool)arg2;
+ (void)MoveAllWindowOutOfScreen:(id)arg1 animation:(_Bool)arg2;
+ (struct CGRect)calcFrameByOrientation:(int)arg1 id:(id)arg2;

@end


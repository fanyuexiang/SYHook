//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NewAssetAccountSettingController;

@interface NewAssetAccountSettingHandleEvent : NSObject
{
    NewAssetAccountSettingController *_controller;
}

- (void).cxx_destruct;
@property(nonatomic) __weak NewAssetAccountSettingController *controller; // @synthesize controller=_controller;
- (void)alertDefaultAccountCanceled:(id)arg1;
- (void)openAssetMiniProgramsAction;
- (void)deleteModel:(id)arg1;
- (void)addModel:(id)arg1;
- (void)queryData;
- (void)backBtnAction;

@end


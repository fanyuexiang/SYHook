//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSDictionary, NSString;

@interface WBSuperGroupObject : NSObject
{
    NSString *_superGroup;
    NSString *_section;
    NSDictionary *_extData;
}

+ (id)superGroupObjectWithDictionary:(id)arg1;
+ (id)dictionaryWithSuperTopicObject:(id)arg1;
+ (id)object;
- (void).cxx_destruct;
@property(retain, nonatomic) NSDictionary *extData; // @synthesize extData=_extData;
@property(retain, nonatomic) NSString *section; // @synthesize section=_section;
@property(retain, nonatomic) NSString *superGroup; // @synthesize superGroup=_superGroup;
- (id)validate;

@end


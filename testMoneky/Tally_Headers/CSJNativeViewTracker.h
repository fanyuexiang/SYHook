//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "CSJViewTracker-Protocol.h"

@class NSDictionary, NSPointerArray, NSString;
@protocol CSJAd;

@interface CSJNativeViewTracker : NSObject <CSJViewTracker>
{
    _Bool _isWitness;
    NSPointerArray *_views;
    id <CSJAd> _admodel;
    NSDictionary *_extra;
    long long _repeatType;
    long long _lastPresentation;
    CDUnknownBlockType _showEvent;
    CDUnknownBlockType _dismissEvent;
    double _showTimestamp;
    NSString *_tag;
}

- (void).cxx_destruct;
@property(copy, nonatomic) NSString *tag; // @synthesize tag=_tag;
@property(nonatomic) double showTimestamp; // @synthesize showTimestamp=_showTimestamp;
@property(copy) CDUnknownBlockType dismissEvent; // @synthesize dismissEvent=_dismissEvent;
@property(copy) CDUnknownBlockType showEvent; // @synthesize showEvent=_showEvent;
@property(nonatomic) long long lastPresentation; // @synthesize lastPresentation=_lastPresentation;
@property(nonatomic) long long repeatType; // @synthesize repeatType=_repeatType;
@property(nonatomic) _Bool isWitness; // @synthesize isWitness=_isWitness;
@property(copy, nonatomic) NSDictionary *extra; // @synthesize extra=_extra;
@property(retain, nonatomic) id <CSJAd> admodel; // @synthesize admodel=_admodel;
@property(retain, nonatomic) NSPointerArray *views; // @synthesize views=_views;
- (_Bool)invalid;
- (_Bool)checkIfViewInScreen;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end


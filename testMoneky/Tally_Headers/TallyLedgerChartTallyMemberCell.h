//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UITableViewCell.h>

@class CBAttributedLabel, SKThemeSettingItem, TallyLedgerChartTallyMemberModel, UIImageView, UILabel, UIView;

@interface TallyLedgerChartTallyMemberCell : UITableViewCell
{
    CBAttributedLabel *_remarkLabel;
    UILabel *_percentLabel;
    UILabel *_amountLabel;
    UILabel *_progressView;
    TallyLedgerChartTallyMemberModel *_model;
    UIImageView *_iconImgView;
    UIView *_lineView;
    SKThemeSettingItem *_curThemeItem;
}

- (void).cxx_destruct;
@property(retain, nonatomic) SKThemeSettingItem *curThemeItem; // @synthesize curThemeItem=_curThemeItem;
@property(retain, nonatomic) UIView *lineView; // @synthesize lineView=_lineView;
@property(retain, nonatomic) UIImageView *iconImgView; // @synthesize iconImgView=_iconImgView;
- (void)updateModel:(id)arg1;
- (void)clearData;
- (double)calculateLabelWidth:(id)arg1;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end


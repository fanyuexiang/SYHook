//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class BaiduMobAdKeyframeGroup, NSNumber, NSString;

@interface BaiduMobAdShapePath : NSObject
{
    _Bool _closed;
    NSString *_keyname;
    NSNumber *_index;
    BaiduMobAdKeyframeGroup *_shapePath;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) BaiduMobAdKeyframeGroup *shapePath; // @synthesize shapePath=_shapePath;
@property(readonly, nonatomic) NSNumber *index; // @synthesize index=_index;
@property(readonly, nonatomic) _Bool closed; // @synthesize closed=_closed;
@property(readonly, nonatomic) NSString *keyname; // @synthesize keyname=_keyname;
- (void)_mapFromJSON:(id)arg1;
- (id)initWithJSON:(id)arg1;

@end

